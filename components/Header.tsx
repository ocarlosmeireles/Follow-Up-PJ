...
import type { Notification, UserProfile, Theme } from '../types';
...
const Header: React.FC<HeaderProps> = ({ 
  ...
  return (
    <header className="bg-[var(--background-secondary)] p-4 flex justify-between items-center sticky top-0 z-30 border-b border-[var(--border-primary)] flex-shrink-0 transition-colors duration-300">
      ...
      <div className="flex justify-end items-center gap-2 sm:gap-4">
        ...
        {userProfile.role !== UserRole.SUPER_ADMIN && (
          <>
            <button
              ...
              className="bg-[var(--background-secondary)] hover:bg-[var(--background-secondary-hover)] text-[var(--text-secondary)] font-semibold py-2 px-4 rounded-lg border border-[var(--border-secondary)] flex items-center transition-colors duration-200 shadow-sm text-sm"
            >
            ...
            <button
              onClick={onAddBudget}
              className="bg-[var(--accent-primary)] hover:bg-[var(--accent-primary-hover)] text-white font-bold py-2 px-4 rounded-lg flex items-center transition-colors duration-200 shadow-sm text-sm"
            >
        ...
        <div className="flex items-center gap-1 border-l border-[var(--border-primary)] pl-2 sm:pl-4">
            {/* Theme Toggle */}
            ...
            <div className="relative" ref={notificationsRef}>
                <button onClick={() => setNotificationsOpen(prev => !prev)} className="p-2 relative rounded-full text-[var(--text-secondary)] hover:bg-[var(--background-tertiary)] transition-colors">
                    ...
                    {notifications.length > 0 && <span className="absolute top-1.5 right-1.5 block w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-white dark:border-[var(--background-secondary)]"></span>}
                </button>
                {isNotificationsOpen && (
                    <div className="absolute top-full right-0 mt-2 w-80 bg-[var(--background-secondary)] rounded-lg shadow-xl border border-[var(--border-primary)] overflow-hidden z-40">
            ...
            <div className="relative" ref={userMenuRef}>
              <button onClick={() => setUserMenuOpen(prev => !prev)} className="flex items-center gap-3 border-l border-[var(--border-primary)] pl-2 sm:pl-4">
                  <div className="text-right hidden sm:block">
                      <span className="font-semibold text-sm text-[var(--text-secondary)]">{userProfile.name}</span>
               ...
               {isUserMenuOpen && (
                    <div className="absolute top-full right-0 mt-2 w-64 bg-[var(--background-secondary)] rounded-lg shadow-xl border border-[var(--border-primary)] overflow-hidden z-40">
                       <div className="p-3 border-b border-[var(--border-primary)] text-left">
                       ...
                       <div className="p-2">
                           <button 
                            ...
                            className="w-full text-left text-sm flex items-center gap-2 p-2 rounded-md text-[var(--text-secondary)] hover:bg-[var(--background-tertiary)]"
                          >
